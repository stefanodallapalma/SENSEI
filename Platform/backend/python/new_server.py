...
def reviews():
    # Vendor alias
    pseudonym_controller = PseudonymizedVendorController()
    vendor_pseudonym = pseudonym_controller.get_vendors_alias()

    # Pseudonymize reviews
    reviews_controller = ReviewController()
    reviews_pseudonym = reviews_controller.get_pseudonymized_reviews()

    product_cleaned_controller = ProductCleanedController()
    unique_timestamps = product_cleaned_controller.get_distinct_timestamps()

    # These timestamps are transformed into their corresponding week as sales are aggregated per week in the graphs
    unique_weeks = list(set([utils.date_formatter(timestamp) for timestamp in unique_timestamps]))
    unique_weeks.sort()

    unique_vendors = list(set([review["name"] for review in reviews_pseudonym]))
    unique_markets = list(set([review["market"] for review in reviews_pseudonym]))
    unique_vendors_markets = {}
    for x in reviews_pseudonym:
    ...
    # Macro category fullfill
    for review in reviews_pseudonym:
        format_date = utils.date_formatter(review["timestamp"])
        if format_date in unique_weeks and review['market'] != 'cannazon':
            if review['macro_category'] != 'No drugs' and review['macro_category'] != 'Unknown':
                markets_ratio[review['market']][format_date]["Drugs"] += 1
            else:
                category = review['macro_category']
                markets_ratio[review['market']][format_date][category] += 1
    ...
    # unique_vendors_dict = {vendor: {week: 0 for week in unique_weeks} for vendor in unique_vendors}
    unique_vendors_dict = {vendor: {week: 0 for week in unique_weeks} for vendor in unique_vendors}

    for review in reviews_pseudonym:
        format_date = utils.date_formatter(review["timestamp"])

        # if format_date in unique_weeks[:-1]:
        if format_date in unique_weeks:
            deals = 1

            if review['market'] == 'cannazon':
                deals = float(review['deals'].strip('€'))

            unique_vendors_dict[review['name']][format_date] += deals

    for vendor, weeks in unique_vendors_dict.items():
        for week, amount in weeks.items():
            if unique_vendors_markets[vendor] != 'cannazon':
                amount *= markets_ratio[unique_vendors_markets[vendor]][week]

            unique_vendors_dict[vendor][week] = round(amount)


    product_price_data = product_cleaned_controller.get_product_cleaned()
    # Pseudonymize the aliases by means of the pseudonyms

    for i in range(len(product_price_data)):
        vendor_name = product_price_data[i]['vendor']
        fake_name = vendor_pseudonym.get(vendor_name, vendor_name)
        product_price_data[i]['vendor'] = fake_name
    ...
    # return jsonify(product_price_data)
