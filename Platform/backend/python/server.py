...
# app.config['MYSQL_USER'] =  'b9a0264095fb9d'
# app.config['MYSQL_PASSWORD'] = 'be8a15cd'
# app.config['MYSQL_DATABASE'] = 'heroku_7d2953bac37192d'
...
def reviews():
    ...
    mycursor.execute(f'SELECT * FROM {db_name}.pseudonymized_vendors;')
    ...
    conn = mysql.connection
    mycursor = conn.cursor()
    mycursor.execute(f'SELECT * FROM {db_name}.reviews;')
    row_headers = [x[0] for x in mycursor.description]
    data = mycursor.fetchall()
    mycursor.close()
    json_data_temp = []
    ...
    conn = mysql.connection
    mycursor = conn.cursor()
    mycursor.execute(f'SELECT distinct timestamp FROM {db_name}.products_cleaned;')
    row_headers = [x[0] for x in mycursor.description]
    ...
    markets_ratio = {market:{week:{product:0 for product in ['Drugs', 'No drugs', 'Unknown']} for week in unique_weeks} for market in unique_markets}
        ...
        if review['market'] != 'cannazon':
            for market, weeks in markets_ratio.items():
                for week, product_types in weeks.items():
                    if review['market'] == market and review['macro_category'] != 'No drugs' and review['macro_category'] != 'Unknown' and week == datetime.datetime.utcfromtimestamp(float(review['timestamp'])).strftime('%Y') + '- week ' + str(datetime.datetime.fromtimestamp(float(review['timestamp'])).isocalendar()[1]):
                        product_types['Drugs'] += 1
                    if review['market'] == market and review['macro_category'] == 'No drugs' and week == datetime.datetime.utcfromtimestamp(float(review['timestamp'])).strftime('%Y') + '- week ' + str(datetime.datetime.fromtimestamp(float(review['timestamp'])).isocalendar()[1]):
                        product_types['No drugs'] += 1
                    if review['market'] == market and review['macro_category'] == 'Unknown' and week == datetime.datetime.utcfromtimestamp(float(review['timestamp'])).strftime('%Y') + '- week ' + str(datetime.datetime.fromtimestamp(float(review['timestamp'])).isocalendar()[1]):
        ...
        if review['market'] != 'cannazon': 
            for vendor, weeks in unique_vendors_dict.items():
                for week, amount in weeks.items():
                    if review['name'] == vendor and review['market'] == unique_vendors_markets[vendor] and week == datetime.datetime.utcfromtimestamp(float(review['timestamp'])).strftime('%Y') + '- week ' + str(datetime.datetime.fromtimestamp(float(review['timestamp'])).isocalendar()[1]):
    ...
    mycursor = conn.cursor()
    mycursor.execute(f'SELECT * FROM {db_name}.products_cleaned;')
    row_headers = [x[0] for x in mycursor.description]
    data = mycursor.fetchall()
    mycursor.close()
        ...
        for country, weeks in countries_weeks.items():
            for week, storage in weeks.items():
                for y in all_products_vendor:
                    if y['ships_from'] == country and week == datetime.datetime.utcfromtimestamp(float(y['timestamp'])).strftime('%Y') + '- week ' + str(datetime.datetime.fromtimestamp(float(y['timestamp'])).isocalendar()[1]):
                ...
                for product in storage:
                    ...
                    if product['macro_category'] not in new_storage:
                        new_storage[product['macro_category']] = {'quantity': 1, 'all_prices': [product['price']], 'average_price': product['price']}
                    else:
                        new_storage[product['macro_category']]['quantity'] += 1
                        new_storage[product['macro_category']]['all_prices'].append(product['price'])
                        new_storage[product['macro_category']]['average_price'] = sum(new_storage[product['macro_category']]['all_prices'])/new_storage[product['macro_category']]['quantity']
                    ...
                    for country, weeks in storing_place.items():
                        for week_products, product_categories in weeks.items():
                            if week_products == next_week and product_categories != {}:
                                for product_category, data in product_categories.items():
                                    storage_to_alter[country][week_products][product_category]['drug category sales'] = data['quantity'] * unique_vendors_dict[vendor][week_sales]/products_sold_week
                else:
                    for country, weeks in storing_place.items():
                        for week_products, product_categories in weeks.items():
                            if week_products == next_week and product_categories != {}:
                                for product_category, data in product_categories.items():
                                    storage_to_alter[country][week_products][product_category]['drug category sales'] = data['quantity'] * data['average_price'] * (unique_vendors_dict[vendor][week_sales]/products_sold_week)
            ...
            for week, categories in weeks.items():
                for category, data in categories.items():
                    if data == {}:
                        ...
                        vendor_country_week_drugs[vendor][country][week][category] = vendor_country_week_drugs[vendor][country][week][category]['drug category sales']
    ...
    for vendor, countries in vendor_country_week_drugs_copy.items():
        for country, weeks in countries.items():
            for week, categories in weeks.items():
                ...
                vendor_country_week_drugs[vendor][country][correct_week] = vendor_country_week_drugs[vendor][country].pop(week)
    ...
    for product in product_price_data:
        for market, weeks in market_week_average_price.items():
            for week, average_price in weeks.items():
                if product['market'] == market and week == datetime.datetime.utcfromtimestamp(float(product['timestamp'])).strftime('%Y') + '- week ' + str(datetime.datetime.fromtimestamp(float(product['timestamp'])).isocalendar()[1]):
                ...
                if vendor_vendor == vendor:
                    for week_vendor, reviews_vendor in weeks_vendor.items():
                        ...
                        if reviews_vendor != 0 and week_vendor not in weeks_sales.keys():
                             unique_vendors_dict[vendor][week_vendor] = reviews_vendor * market_week_average_price[market][week]
        ...
        for treemap_market, treemap_d in treemap_growth.items():
            if treemap_market == market:
                ...
                if average_sales_second_half == 0:
                    ...
                    growth_rate = ((average_sales_second_half - average_sales_first_half)/ average_sales_first_half) * 100
                    if growth_rate > 200:
    ...
    return jsonify(unique_vendors_dict, markets_total_sales, dates_country_final, final_dict, unique_weeks[:-1],treemap_data,treemap_sales,treemap_growth,market_vendor_weeks_sales,scatterplot_final,product_page_function_1,product_page_function_2,treemap_growth_copy)
...
def product_page():
    ...
    drugs_mapping = {'Benzodiazepines': ['benzodiazepines','klonopin', 'aprazolam','alprazolam','xanax','lorazepam', 'flunitrazolam', 'diazepam', 'valium'],
    ...
    final_dict = {country:{drug:{week:0 for week in unique_weeks[:-1]} for drug in drugs_mapping.keys()} for country in unique_countries + ['All']}
    final_dict_markets = {market:{country:{drug:{week:0 for week in unique_weeks[:-1]} for drug in drugs_mapping.keys()} for country in unique_countries + ['All']} for market in unique_markets + ['All markets']}
                    ...
                    for country_final, categories_final in final_dict.items():
                        for category_final, weeks_final in categories_final.items():
                            for week_final, amount_final in weeks_final.items():
                                if (country == country_final or country_final == 'All') and week == week_final and category == category_final:
                        ...
                        for country_final_markets, categories_final_markets in final_dict.items():
                            for category_final_markets, weeks_final_markets in categories_final_markets.items():
                                for week_final_markets, amount_final_markets in weeks_final_markets.items():
                                    if (country == country_final_markets or country_final_markets == 'All') and week == week_final_markets and category == category_final_markets and (unique_vendors_markets[vendor] == market_final_markets or market_final_markets == 'All markets'):
                                    
                                        final_dict_markets[market_final_markets][country_final_markets][category_final_markets][week_final_markets] += amount
                    ...
                    if quantities != {}:
                        for vendor2, timestamps in vendor_weekly_sales_2.items():
                            for timestamp, data in timestamps.items():
                                if category not in vendor_weekly_sales_2[vendor][timestamp] and vendor == vendor2 and week == datetime.datetime.utcfromtimestamp(float(datetime.datetime.strptime(timestamp, '%d-%m-%Y').strftime("%s"))).strftime('%Y') + '- week ' + str(datetime.datetime.fromtimestamp(float(datetime.datetime.strptime(timestamp, '%d-%m-%Y').strftime("%s"))).isocalendar()[1]):
    ...
    specialist_generalist = {country:{specialist:{week:{market:[] for market in unique_markets} for week in unique_dates} for specialist in ['Specialist', 'Generalist']} for country in unique_countries + ['All']}
                                ...
                                for timestamp, markets in timestamps.items():
                                    for market, data_storage in markets.items():
                                        ...
                                        if len(all_categories_list) == 1:
                                            if (country == country2 or country2 == 'All') and market == unique_vendors_markets[vendor] and week == datetime.datetime.utcfromtimestamp(float(datetime.datetime.strptime(timestamp, '%d-%m-%Y').strftime("%s"))).strftime('%Y') + '- week ' + str(datetime.datetime.fromtimestamp(float(datetime.datetime.strptime(timestamp, '%d-%m-%Y').strftime("%s"))).isocalendar()[1]):
                                                if vendor not in specialist_generalist[country2]['Specialist'][timestamp][market]:
                                                    specialist_generalist[country2]['Specialist'][timestamp][market].append(vendor)
                                        # If a vendor sells products in multiple categories --> generalist
                                        if len(all_categories_list) > 1:
                                            if (country == country2 or country2 == 'All') and market == unique_vendors_markets[vendor] and week == datetime.datetime.utcfromtimestamp(float(datetime.datetime.strptime(timestamp, '%d-%m-%Y').strftime("%s"))).strftime('%Y') + '- week ' + str(datetime.datetime.fromtimestamp(float(datetime.datetime.strptime(timestamp, '%d-%m-%Y').strftime("%s"))).isocalendar()[1]):
                                                if vendor not in specialist_generalist[country2]['Generalist'][timestamp][market]:
                                                    specialist_generalist[country2]['Generalist'][timestamp][market].append(vendor)
    # Structure of the output:
    ...
    return jsonify(final_dict_markets, vendor_weekly_sales,all_categories,vendor_weekly_sales_2,drugs_mapping,specialist_generalist)
...
def product_data():
    ...
    country_timestamp_market = {country:{timestamp:{market:[] for market in unique_markets_list} for timestamp in unique_dates_list } for country in unique_countries_list}
    temp = {country:{timestamp:{market:[] for market in unique_markets_list} for timestamp in unique_dates_list } for country in unique_countries_list}
    for country, timestamps in temp.items():
        for timestamp, markets in timestamps.items():
            for market, storing_place in markets.items():
                ...
                for product in json_data:
                    if product['ships_from'] == country and product['timestamp'] == timestamp and product['market'] == market:
    ...
    country_timestap_market_type = {country:{timestamp:{market:[] for market in unique_markets_list} for timestamp in unique_dates_list } for country in unique_countries_list}
    ...
    final_dict = {country:{vendor_type:{date:{market:[] for market in unique_markets_list} for date in unique_dates_list} for vendor_type in ['Distributor', 'Both','End user' ]} for country in unique_countries_list}
    ...
    y_axis_height = {country:{market:{date:0 for date in unique_dates_list} for market in unique_markets_list} for country in unique_countries_list}
...
def nlp():
    ...
    mycursor = mysql.new_cursor()
    mycursor.execute(f"SELECT timestamp,market,name,`group/individual`, ships_from FROM {db_name}.`vendor-analysis`;")
    row_headers = [x[0] for x in mycursor.description]
    data = mycursor.fetchall()
    json_data_temp = []
    for line in data:
    ...
    final_dictionary = {country:{group:{date:{market:[] for market in unique_markets_list} for date in unique_dates_list } for group in ['Group', 'Individual', 'Unknown']} for country in unique_countries_list}
    for record in good_labelled_data:
        final_dictionary[record['ships_from']][record['group/individual']][record['timestamp']][record['market']].append(record['name'])
        if record['name'] not in final_dictionary['All'][record['group/individual']][record['timestamp']][record['market']]:
            final_dictionary['All'][record['group/individual']][record['timestamp']][record['market']].append(record['name'])
...
def operational():
    ...
    conn = mysql.connection
    mycursor = conn.cursor()
    mycursor.execute('SELECT `vendor-analysis`.timestamp, `vendor-analysis`.market, `vendor-analysis`.name, '
                     '`vendor-analysis`.email, `vendor-analysis`.`phone number`, `vendor-analysis`.wickr, '
                     '`vendor-analysis`.`group/individual`, `vendor-analysis`.`other markets`, '
                     '`vendor-analysis`.ships_from, `vendor-analysis`.ships_to, `vendor-analysis`.normalized_score, '
                     f'`vendor-analysis`.registration_date FROM {db_name}.`vendor-analysis`;')
    row_headers = [x[0] for x in mycursor.description]
    data = mycursor.fetchall()
    mycursor.close()
    ...
    df_duplicates = pd.DataFrame(json_data_start, columns=['timestamp', 'market', 'name', 'email', 'phone number', 'wickr', 'group/individual', 'other markets', 'ships_from', 'ships_to', 'score_normalized', 'registration'])
    ...
    shipping_locations = {market:{country:{x:{week:[] for week in unique_dates} for x in ['One country', 'Multiple countries']} for country in unique_countries + ['All']} for market in unique_markets + ["All markets"]}
    shipping_locations_copy = {market:{country:{x:{week:[] for week in unique_dates} for x in ['One country', 'Multiple countries', 'Unknown']} for country in unique_countries + ['All']} for market in unique_markets + ["All markets"]}
                    ...
                    for multiple, weeks in multiples.items():
                        for week, vendors in weeks.items():
                            ...
                            if multiple_single_country > 1:
                                ...
                                if (country in record['ships_from'].split(',') or country == "All") and (market == record['market'] or market =="All markets") and week == record['timestamp'] and record['name'] not in shipping_locations[market][country]['Multiple countries'][week]:
                                    shipping_locations[market][country]['Multiple countries'][week].append(record['name'])
                                    continue
                            if multiple_single_country == 1:
                                if (country == record['ships_from'] or country == "All") and (market == record['market'] or market == "All markets") and week == record['timestamp'] and record['name'] not in shipping_locations[market][country]['One country'][week]:
    ...
    return jsonify(final_json_data_vendor_final, country_values, unique_countries,markets,timestamps,all_large_vendors,all_growing_vendors,shipping_locations)
...
def raw_data():
    ...
    mycursor = mysql.new_cursor()
    mycursor.execute(f"SELECT message, name, market, product, timestamp FROM {db_name}.reviews;")
    row_headers = [x[0] for x in mycursor.description]
    data = mycursor.fetchall()
...
def network_data():
    ...
    #Import actual data    
    mycursor = mysql.new_cursor()
    mycursor.execute(f"SELECT timestamp, market, name, `other markets` FROM {db_name}.`vendor-analysis`")
    row_headers = [x[0] for x in mycursor.description]
    data = mycursor.fetchall()
    json_data_temp = []
    for line in data:
    ...
    for timestamp, network_data_input in timestamp_dict_final.items():
        ...
        for market in market_occurence_iterable:
            right_format["nodes"].append({'id': counter, 'value':market[1], 'label':market[0],'title':'{} active vendors'.format(market[1]), 'color':'#FFFFFF'})
            counter += 1
    ...
    return jsonify(timestamp_dict_final, which_market_final,which_market_operational_table,operational_table_structure_keys,multiple_one_market)
