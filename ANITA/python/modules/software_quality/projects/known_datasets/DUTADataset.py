...
from .KnownDataset import KnownDataset
from utils.FileUtils import getfiles

name = "Duta"
duta_info = "DUTA_10K.csv"
duta_json = "duta.json"
delimiter = ","


class DUTADataset(KnownDataset):
    def __init__(self):
        super().__init__(name)

    @property
    def duta_csv_path(self):
        return os.path.join(self.path, duta_info)

    def generate_additional_info(self, *args):
        if len(args) != 2:
            raise Exception("The argument passed must be a path")

        if not os.path.isdir(args[0]):
            raise Exception("The path must be a directory")

        # Load first additional data
        duta_dataframe = pd.read_csv(self.duta_csv_path, delimiter=delimiter)

        final_dataframe = extra_features(duta_dataframe, args[0], args[1])
        final_dataframe.to_json(self.json_path, orient='records')

    def merge(self, original_json, additional_json):
        pass


def extra_features(dataframe, html_path, pages=None):
    ...
    for index, row in dataframe.iterrows():
        ...
        try:
            ...
            if onion_address in pages:
                print("\t" + onion_address)
                path = os.path.join(html_path, onion_address + ".html")
