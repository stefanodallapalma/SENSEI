...
def retrieve_metrics(project_key, metric_list=None, wait=False):
    ...
    for sub_metric_list in sub_metric_lists:
        ...
        number_html_pages = server_sq.get_json_content(response)["paging"]["total"]
        ...
        for x in range(max_pages):
            ...
            components = server_sq.get_json_content(response)["components"]
...
def pending_tasks(project_key):
    ...
    project_tasks_content = server_sq.get_json_content(project_tasks)
